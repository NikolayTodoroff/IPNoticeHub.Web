...
namespace IPNoticeHub.Infrastructure.Persistence.Repositories.TrademarkRepository
{
    public class TrademarkStatusSnapshotRepository : ITrademarkStatusSnapshotRepository
    {
        ...
        public async Task<(int? StatusCodeRaw, string StatusDetail, DateTime? StatusDateUtc)?> 
            GetStatusSnapshotAsync(
            int trademarkId, 
            CancellationToken 
            cancellationToken)
        {
            ...
            if (snapshotResult == null) return null;

            return (
                snapshotResult.StatusCodeRaw, 
                snapshotResult.StatusDetail ?? string.Empty, 
                snapshotResult.StatusDateUtc);
        }
    }
}
