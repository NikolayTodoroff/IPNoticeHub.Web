...
{
    /// <summary>
    /// Section: TrademarkCollectionService – Edge Cases
    /// - Verifies that AddAsync with unknown PublicId does not create a new link.
    /// - Verifies that AddAsync does not duplicate rows when link already active.
    /// - Ensures that RemoveAsync when not in collection → no-op (no row created/changed).
    /// </summary>
    [TestFixture]
    public class TmCollectionServiceEdgeCaseTests
    {
        [Test]
