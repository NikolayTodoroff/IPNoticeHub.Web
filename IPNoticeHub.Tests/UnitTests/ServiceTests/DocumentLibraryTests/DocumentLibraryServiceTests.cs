    ...
    public class DocumentLibraryServiceTests
    {
        ...
        public async Task SaveDocumentAsync_GeneratesDefaultTitle_WhenTitleMissing()
        {
            ...
            var dto = new DocumentCreateDto
            {
                ...
                LetterDate = new DateTime(2025, 12, 8, 0, 0, 0, DateTimeKind.Utc),
                BodyTemplate = "Hello {{RecipientName}}, this concerns {{WorkTitle}} {{RegistrationNumber}}."
            };
            ...
            repository.Setup(
                r => r.AddAsync(It.IsAny<LegalDocument>(), It.IsAny<CancellationToken>()))
                .Callback<LegalDocument, CancellationToken>((ld, _) => entity = ld)
                .ReturnsAsync(42);

            var pdfService = new Mock<IPdfService>(MockBehavior.Loose);
            var logger = new Mock<DocumentLibraryService>(MockBehavior.Loose);

            var sut = new DocumentLibraryService(repository.Object, pdfService.Object);
            ...
            repository.Verify(r => r.AddAsync(
                It.IsAny<LegalDocument>(), It.IsAny<CancellationToken>()), Times.Once);
        }

        [Test]
        public async Task RestoreDocumentSnapshotAsync_ReturnsPdfAndFileName_ForTrademarkCeaseAndDesist()
        {
            var userId = "user-123";
            var documentId = 7;
            ...
            repository.Setup(r => r.GetDocumentByIdAsync(
                documentId, userId, It.IsAny<CancellationToken>()))
            ...
            pdfService
                ...
                .Callback<CeaseDesistInput, CancellationToken>((input, _) => capturedInput = input)
            ...
            capturedInput!.SenderName.Should().Be(document.SenderName);
            capturedInput.SenderAddress.Should().Be(document.SenderAddress);
            capturedInput.RecipientName.Should().Be(document.RecipientName);
            capturedInput.RecipientAddress.Should().Be(document.RecipientAddress);
            capturedInput.WorkTitle.Should().Be(document.IpTitle);
            capturedInput.RegistrationNumber.Should().Be(document.RegistrationNumber);
            capturedInput.Date.Should().Be(document.LetterDate);
            capturedInput.AdditionalFacts.Should().Be(document.AdditionalFacts);
            capturedInput.BodyTemplate.Should().Be(document.BodyTemplate);
        ...
        public async Task RestoreDocumentSnapshotAsync_ReturnsNull_WhenDocumentNotFound()
        {
            ...
            var repo = new Mock<IDocumentLibraryRepository>(MockBehavior.Strict);
            repo.Setup(r => r.GetDocumentByIdAsync(documentId, userId, It.IsAny<CancellationToken>()))
                .ReturnsAsync((LegalDocument?)null);

            var pdfService = new Mock<IPdfService>(MockBehavior.Loose);
            var logger = new Mock<DocumentLibraryService>(MockBehavior.Loose);

            var sut = new DocumentLibraryService(repo.Object, pdfService.Object);

            var result = await sut.RestoreDocumentSnapshotAsync(documentId, userId, CancellationToken.None);
